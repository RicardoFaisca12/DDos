...
GetOptions(
	...
	"delay=f"=> \$delay,		# intser-packet delay
	"help|?" => \$help);		# help
...
if ($help || !$ip) {
  print <<'EOL';
 The usage of command is perl katchau.pl a.b.c.d
EOL
  exit(1);
}
...
\_______)|/     \||/     \|   \_/   
                                    \n";
printf "[0;31m>> atacando o ip    \n";
printf "[0;36m>> atacando as portas     \n";
($size ? "$size-byte" : "") . " " . ($time ? "" : "") . "\033[1;32m\033[0m\n\n";
print "Interpacket delay $delay msec\n" if $delay;
print "total IP bandwidth $bw kbps\n" if $bw;
printf "[1;31m>> Pressiona CTRL+C para parar o ataque \n" unless $time;
